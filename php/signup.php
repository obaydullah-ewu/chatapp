<?php
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($password)){
        // let's check user email valid or not
        if(filter_var($email, FILTER_VALIDATE_EMAIL)) //if email is valid
        {
            // let's check the email already exist in the database or not
            ...
            if(mysqli_num_rows($sql) > 0)
            {
                ...
                if(isset($_FILES['image']))
                {
                    ...
                    $img_ext = end($img_explode); //here we get the extension of an user uploaded ing file
                    ...
                    if (in_array($img_ext, $extensions) === true){ //if user uploaded img ext is matched with any array extensions
                        ...
                        if(move_uploaded_file($tmp_name, "images/".$new_img_name)){  //if upload img move to our folder successfully
...
//
                        ...
                        echo "Please select an Image file - jpg, jpg, png!";
                    }
                }else{
